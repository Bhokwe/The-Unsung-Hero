...
-**Entry 13:** Integrated the Pygame Mixer module to handle audio. Wrote a safe loading function (play_music) to handle the MusicGPT .mp3 files without crashing. Built a dynamic audio trigger that automatically swaps from the "Triumphant" main theme to the "Frantic" combat theme when tank_phase_active triggers at 45 seconds, perfectly juxtaposing the escalation in gameplay stress. Also added an audio dimming effect upon triggering the Game Over state.
## Reflection Notes
